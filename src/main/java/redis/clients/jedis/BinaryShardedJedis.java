...
public class BinaryShardedJedis extends Sharded<Jedis, JedisShardInfo>
	...
	@Override
	public String ldbSet(byte[] key, byte[] value) {
	...
	@Override
	public Map<byte[], byte[]> ldbIterFowardsKeys(byte[] key, long count) {
		Jedis j = getShard(key);
		return j.ldbIterFowardsKeys(key, count);
	}

	@Override
	public Set<byte[]> ldbIterFowardsKeysAndValues(byte[] key, long count) {
		Jedis j = getShard(key);
		return j.ldbIterFowardsKeysAndValues(key, count);
	}
